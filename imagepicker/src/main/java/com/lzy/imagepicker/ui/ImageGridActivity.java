...
//        mImageGridAdapter = new ImageGridAdapter(this, null);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_PERMISSION_STORAGE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                showInfoToast(getString(R.string.ip_ImageGridActivity_text1));
            }
        } else if (requestCode == REQUEST_PERMISSION_CAMERA) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                imagePicker.takePicture(this, ImagePicker.REQUEST_CODE_TAKE);
            } else {
                showInfoToast(getString(R.string.ip_ImageGridActivity_text2));
            }
        }
    }
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.btn_ok) {
            if (imagePicker.getSelectedImages() == null || imagePicker.getSelectedImages().isEmpty()) {
                showInfoToast(getString(R.string.ip_ImageGridActivity_text3));
                return;
            }
