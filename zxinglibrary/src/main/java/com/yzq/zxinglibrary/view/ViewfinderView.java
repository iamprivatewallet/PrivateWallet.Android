...
public final class ViewfinderView extends View {
    ...
    private void initPaint() {
        ...
        int color = Color.parseColor("#EC125E");

        /*四个角的画笔*/
        reactPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        reactPaint.setColor(color);
        reactPaint.setStyle(Paint.Style.FILL);
        reactPaint.setStrokeWidth(dp2px(4));
...
//        canvas.drawRect(0, frame.bottom + 1, width, height, paint);
    ...
    private void drawFrameBounds(Canvas canvas, Rect frame) {
        ...
        Rect expensionRange = new Rect(frame.left - out,
                frame.top - out,
                frame.right + out,
                frame.bottom + out);

        int corLength = (int) dp2px(20);
        int corWidth = (int) dp2px(2);
        int length = dp2px(10);
        corWidth = corWidth > 15 ? 15 : corWidth;

        Path leftTopPath = new Path();
        leftTopPath.moveTo(expensionRange.left, expensionRange.top + corLength + length);
        leftTopPath.lineTo(expensionRange.left, expensionRange.top + corLength);
        leftTopPath.quadTo(expensionRange.left, expensionRange.top, expensionRange.left + corLength, expensionRange.top);
        leftTopPath.lineTo(expensionRange.left + corLength + length, expensionRange.top);
        canvas.drawPath(leftTopPath, reactPaint);


        Path rightTopPath = new Path();
        rightTopPath.moveTo(expensionRange.right, expensionRange.top + corLength + length);
        rightTopPath.lineTo(expensionRange.right, expensionRange.top + corLength);
        rightTopPath.quadTo(expensionRange.right, expensionRange.top, expensionRange.right - corLength, expensionRange.top);
        rightTopPath.lineTo(expensionRange.right - corLength - length, expensionRange.top);
        canvas.drawPath(rightTopPath, reactPaint);


        Path leftBottomPath = new Path();
        leftBottomPath.moveTo(expensionRange.left, expensionRange.bottom - corLength - length);
        leftBottomPath.lineTo(expensionRange.left, expensionRange.bottom - corLength);
        leftBottomPath.quadTo(expensionRange.left, expensionRange.bottom, expensionRange.left + corLength, expensionRange.bottom);
        leftBottomPath.lineTo(expensionRange.left + corLength + length, expensionRange.bottom);
        canvas.drawPath(leftBottomPath, reactPaint);


        Path rightBottomPath = new Path();
        rightBottomPath.moveTo(expensionRange.right, expensionRange.bottom - corLength - length);
        rightBottomPath.lineTo(expensionRange.right, expensionRange.bottom - corLength);
        rightBottomPath.quadTo(expensionRange.right, expensionRange.bottom, expensionRange.right - corLength, expensionRange.bottom);
        rightBottomPath.lineTo(expensionRange.right - corLength - length, expensionRange.bottom);
        canvas.drawPath(rightBottomPath, reactPaint);
