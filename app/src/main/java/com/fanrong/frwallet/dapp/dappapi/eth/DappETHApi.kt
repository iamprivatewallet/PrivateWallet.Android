package com.fanrong.frwallet.dapp.dappapi.eth
...
object DappETHApi {
    ...
    open class Eth_sendTransaction : FuncHandler {
        ...
        override fun excute(params: Any?): Any {
            ...
            transaction.gasPrice = ethApi.gasPrice(GasPriceReq()).execute().body()!!.getGasPrice()
            transaction.nonce = TransferUtils.getRealNonce(tokenInfo!!,ethApi.getNoce(GetNoceReq(transaction.from)).execute().body()!!.getNoce())
            ...
            fun sendTransfer(sign: String?) {
                SWLog.e(sign)
                thread {
                    val hash = ethApi.ethTransfer(EthTransferReq("0x" + CallJsCodeUtils.readStringJsValue(sign))).execute().body()!!.getHash()
