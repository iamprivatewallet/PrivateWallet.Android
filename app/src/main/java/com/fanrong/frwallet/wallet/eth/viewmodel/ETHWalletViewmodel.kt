...
class ETHWalletViewmodel : WalletViewmodel() {
        ...
        viewModelScope.tryLaunch {
            for (coinBean in coin) {
                if (coinBean.contract_addr.checkNotEmpty()) {
                    var balanceResult =
                        ethApi.susQueryEthTokenBalance(QueryEthTokenBalanceReq(coinBean.sourceAddr!!, coinBean.contract_addr!!)).body()!!
                    coinBean.balance = balanceResult.getBalance().extHexWei2Ten2EtherKeep2()
                } else {
                    var balanceResult = ethApi.susQueryEthBalance(QueryEthBalanceReq(coinBean.sourceAddr!!)).body()!!
                    coinBean.balance = balanceResult.getBalance().extHexWei2Ten2EtherKeep2()
                }
